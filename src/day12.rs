...
pub fn find_nearest_location(locations: &[Location]) -> Result<&Location, Box<dyn Error>> {
    ...
    locations
        .iter()
        .filter(|l| l.density() >= 1000.0)
        .min_by(|a, b| {
            distance_to_location(a)
                .partial_cmp(&distance_to_location(b))
                .unwrap_or(Ordering::Equal)
        })
        .ok_or("No locations found".into())
}

fn distance_to_location(location: &Location) -> f64 {
    (location.x.powi(2) + location.y.powi(2)).sqrt()
}
